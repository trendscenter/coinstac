...
const COMPUTATION_TIMEOUT = 6400000;
const COMPUTATION_DOWNLOAD_TIMEOUT = 40000;
const USER_ID = 'test1';
const PASS = 'password';
const CONS_NAME = 'e2e-consortium-local';
const CONS_DESC = 'e2e-description-local';
const PIPE_NAME = 'e2e-pipeline-local';
const PIPE_DESC = 'e2e-pipeline-description-local';
const COMPUTATION_NAME = 'fMRI Preprocessing Pipeline (New)';
...
describe('e2e run computation with 1 member', () => {
  ...
  it('map data to consortium', async () => {
    ...
    await appWindow.click('button:has-text("Select Directory(s)")', { timeout: EXIST_TIMEOUT });

    //await appWindow.fill('input.MuiInputBase-input[type="number"]', 0);
  ...
  it('displays computation progress', async () => {
    await appWindow.click('a:has-text("Home")', { timeout: COMPUTATION_TIMEOUT });
  ...
  it('displays results', async () => {
    ...
    // Assert
    return Promise.all([
      appWindow.waitForSelector(`h6:has-text("Results: ${CONS_NAME}")`, {
        state: 'visible',
        timeout: EXIST_TIMEOUT,
      }).should.eventually.not.equal(null),
    ]);
