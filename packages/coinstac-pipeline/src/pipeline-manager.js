...
module.exports = {
  ...
  async create({
     ...
     */
    const cleanupPipeline = (activePipelines, remoteClients, runId) => {
      return Promise.all([
    ...
    const managerFactory = ({
       ...
       * @param  {Object} clients      a list of client IDs particapating in pipeline
       ...
       */
        startPipeline({
          ...
          store[clientId].put(`${runId}-profiling`, clientId, {});
          ...
          const pipelineProm = Promise.all([
            ...
            .then(() => {
              ...
              if (!activePipelines[runId].finalTransferList
                ...
                return cleanupPipeline(activePipelines, remoteClients, runId)
                  .then(() => { return res; });
              }
    ...
    if (mode === 'remote') {
      ...
      const nodes = Array.isArray(clientId) ? clientId : [clientId];

      pipes = await nodes.reduce(async (pipes, node) => {
        activePipelines[node] = {};
        ({
          communicate,
          publishData,
        } = await setupOuter({
          ...
          mqttRemoteWSPort,
          mqttRemoteWSPathname,
          activePipelines: activePipelines[node],
          debugProfileClient,
          store,
          remoteProtocol,
          remoteURL,
    ...
    return Array.isArray(clientId) ? pipes : pipes[clientId];
  },
};
