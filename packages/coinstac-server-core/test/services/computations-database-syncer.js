...
test('seed diffing', (t) => {
  t.deepEqual(
    syncService._getComputationsDiff([], []),
    { add: [], update: [], delete: [] },
  ...
  t.deepEqual(
    syncService._getComputationsDiff(mockGoldenSetA, []),
  ...
  t.deepEqual(
    syncService._getComputationsDiff(mockGoldenSetA, [{ name: 'b', tags: ['z'] }]),
    { add: [mockGoldenSetA[0]], update: [mockGoldenSetA[1]], delete: [] },
    'update diff generated'
  );

  t.deepEqual(
    syncService._getComputationsDiff(mockGoldenSetA, [{ name: 'c', tags: ['c'] }]),
